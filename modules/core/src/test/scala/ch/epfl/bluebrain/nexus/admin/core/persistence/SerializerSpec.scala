...
import ch.epfl.bluebrain.nexus.admin.core.projects.Label
...
import ch.epfl.bluebrain.nexus.admin.core.resources.{Ref, ResourceEvent}
...
class SerializerSpec extends WordSpecLike with Matchers with Inspectors with ScalatestRouteTest with TestHeplers {
  ...
  "A Serializer" when {
    ...
    val projectLabel  = Label("projectid").get
    val projectUriRef = Ref(Label.labelId.uri(projectLabel), 1L)
    ...
    "using EventSerializer" should {
      val results = List(
        DataAndJson(
          ResourceCreated(projectUriRef,
                          meta,
                          tags,
                          Value(Json.obj("key" -> Json.fromString("http://localhost.com/path/")), Config(10L)).asJson),
          s"""{"ref":{"id":"https://bbp.epfl.ch/nexus/projects/projectid","rev":1},"meta":{"author":{"id":"realms/realm/users/sub:1234","type":"UserRef"},"instant":"${meta.instant}"},"tags":["project"],"value":{"@context":{"key":"http://localhost.com/path/"},"config":{"maxAttachmentSize":10}},"type":"ResourceCreated"}"""
        ),
        DataAndJson(
          ResourceUpdated(
            projectUriRef,
          ...
          s"""{"ref":{"id":"https://bbp.epfl.ch/nexus/projects/projectid","rev":1},"meta":{"author":{"id":"realms/realm/users/sub:1234","type":"UserRef"},"instant":"${meta.instant}"},"tags":["project"],"value":{"@context":{"key2":"http://localhost.com/path2/"},"config":{"maxAttachmentSize":20}},"type":"ResourceUpdated"}"""
        ),
        DataAndJson(
          ResourceDeprecated(projectUriRef, meta, tags),
          s"""{"ref":{"id":"https://bbp.epfl.ch/nexus/projects/projectid","rev":1},"meta":{"author":{"id":"realms/realm/users/sub:1234","type":"UserRef"},"instant":"${meta.instant}"},"tags":["project"],"type":"ResourceDeprecated"}"""
        )
      )
